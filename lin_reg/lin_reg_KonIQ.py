...
def evaluate_bvqa_one_split(i, X, y, args):
    ...
    if args.dataset != "FLIVE":
        ...
        print(f'Split {i}: X_train shape: {X_train.shape}, X_test shape: {X_test.shape}, y_train shape: {y_train.shape}, y_test shape: {y_test.shape}')

        X_train_reduced, X_validation, y_train_reduced, y_validation = train_test_split(
            X_train, y_train, test_size=0.125, random_state=np.random.randint(low=0, high=100, size=1)[0])
        print(f'Split {i}: X_train_reduced shape: {X_train_reduced.shape}, X_validation shape: {X_validation.shape}, y_train_reduced shape: {y_train_reduced.shape}, y_validation shape: {y_validation.shape}')
    else:
        test_split_idx = np.load("/work/08129/avinab/ls6/ContrastiveImage/CONTRIQUE/Evaluation_Datasets/FLIVE/test.npy")
        train_val_idx = np.load("/work/08129/avinab/ls6/ContrastiveImage/CONTRIQUE/Evaluation_Datasets/FLIVE/train_val.npy")
        ...
        X_train_reduced, X_validation, y_train_reduced, y_validation = train_test_split(
            X_train, y_train, test_size=0.205, random_state=np.random.randint(low=0, high=100, size=1)[0])
        print(f'Split {i}: X_train_reduced shape: {X_train_reduced.shape}, X_validation shape: {X_validation.shape}, y_train_reduced shape: {y_train_reduced.shape}, y_validation shape: {y_validation.shape}')
...
def main(args):
    X = np.load("KonIQ_features_depth.npy")
    y = np.load("KonIQ_scores.npy")
